...
#define NSPECIES        99         /* max # of species */
...
#define NGENE           16000      /* max # of loci */
...
struct CommonInfo {
   ...
   char oldconP[NS * 2 - 1];
   int  cleandata, checkpoint, verbose;
...
/* spname[] has format "12", "234", etc., where the index refers to those in control file.
...
int MakeDiploid(unsigned char *z[], unsigned char *zt[], int ns, int nsHet, int ls);
         ...
         for (i = 0, t = 0, pline = line; i < lline&&line[i]; i++)
            ...
            for (iopt = 0; iopt < nopt; iopt++) {
               if (strncmp(opt, optstr[iopt], 12) == 0) {
                  ...
                  case (21):
                     sscanf(pline + 1, "%d:%lf%lf%lf%lf%lf%lf%lf", &mcmc.resetsteps, eps, eps + 1, eps + 2, eps + 3, eps + 4, eps + 5, eps + 6);
                     break;
                  case (22):
...
double mtMRCA = 0, meanmigrations[(NSPECIES * 2 - 1)*(NSPECIES * 2 - 1)];
   ...
   for (T = 0; ; npop--) {   /*  # of epochs */
      ...
      for (; ; ) {
         ...
         if (r < C) {  /* coalescent in pop i of lineages k1 & k2 */
            ...
            meanmigrations[ipop[j] * (stree.nspecies * 2 - 1) + ipop[i]] ++;
...
int MakeDiploid(unsigned char *z[], unsigned char *zt[], int ns, int nsHet, int ls)
{
   /* This takes ns sequences in z[ns][] and produces nsHet sequences in zt[nHet], with
   heterozygotes generated using data.diploid[].  Sequences in z[] are already encoded as 0 1 2 3.
   This changes com.spname[].
   */
   int i, j, k, h, is = 0, isHet = 0;
   unsigned char b[2], c[2], seqname[96], *pch;

   for (i = 0; i < stree.nspecies; i++) {
      for (j = 0; j < stree.nseqsp[i] / (1 + data.diploid[i]); j++) {
         if (data.diploid[i] == 0) {
            memcpy(zt[isHet], z[is], ls * sizeof(unsigned char));
            is++;  isHet++;
         }
         else {
            memcpy(zt[isHet], z[is], ls * sizeof(unsigned char));
            for (h = 0; h < ls; h++) {
               ...
               if (b[0] != b[1]) {
                  c[0] = min2(b[0], b[1]);
                  c[1] = max2(b[0], b[1]);
                  if (c[0] == 0 && c[1] == 1)         zt[isHet][h] = 5;   /* TC = Y */
                  else if (c[0] == 2 && c[1] == 3)    zt[isHet][h] = 6;   /* AG = R */
                  else if (c[0] == 1 && c[1] == 2)    zt[isHet][h] = 7;   /* CA = M */
                  else if (c[0] == 0 && c[1] == 3)    zt[isHet][h] = 8;   /* TG = K */
                  else if (c[0] == 1 && c[1] == 3)    zt[isHet][h] = 9;   /* CG = S */
                  else if (c[0] == 0 && c[1] == 2)    zt[isHet][h] = 10;  /* TA = W */
               }
            }
            is += 2;  isHet++;
         }
...
void SimulateData(void)
{
/* Always allocate com.z[], spnameFull[].  
   Allocate zt[nsFULL], spnameHet[nsFULL] for diploid data only, if (nsHet<nsFull).  
   The size is nsFull as the space is used for random phase resolution as well.
   For diploid data, concatenated file has nsHet sequences, but the tree is for nsFull sequences.  
   Also the program prints out mydatafile_full.txt (with the true phase resolution), and 
   mydatafile_rand.txt (with a random phase resolution).
*/
   char **zconcat, **zt, **spnameFull, **spnameHet;
   FILE *fseq = NULL, *ftree = NULL, *fImap = NULL, *fconcat = NULL, *fmodelpara = NULL;
   FILE *fseqfull = NULL, *fseqrand = NULL;
   double rlocus, y, snoderates[NSPECIES * 2 - 1];
   int nloci = com.ngene, locus, i, j, k, h, iGtree, maxls = com.ls, lspname=50;
   ...
   for (i = 0, nsHet = 0; i < stree.nspecies; i++)
      if (data.diploid[i])  nsHet += stree.nseqsp[i] / 2;
      else                  nsHet += stree.nseqsp[i];
         
   com.cleandata = 1;
   for (i = 0; i < stree.nspecies; i++)  if (data.diploid[i]) com.cleandata = 0;

   spnameFull = (char**)malloc(nsFull * sizeof(char*));
   if (spnameFull == NULL) error2("oom z full");
   for (i = 0; i < nsFull; i++) {
      spnameFull[i] = (char*)malloc(lspname * sizeof(char));
      if (spnameFull[i] == NULL) error2("oom z full 1");
   }
   if (nsHet < nsFull) {
      spnameHet = (char**)malloc(nsFull * sizeof(char*));
      if (spnameHet == NULL) error2("oom spnameHet");
      for (i = 0; i < nsFull; i++)
         if ((spnameHet[i] = (char*)malloc(lspname * sizeof(char))) == NULL) error2("oom spnameHet 1");
   }
   if (com.seqf && com.seqf[0] && nsHet < nsFull) {
      if ((zt = (unsigned char**)malloc(nsFull * sizeof(unsigned char*))) == NULL) error2("oom zt");
      for (i = 0; i < nsFull; i++)
         if ((zt[i] = (unsigned char*)malloc(com.ls * sizeof(unsigned char))) == NULL) error2("oom zt 1");
   }

   if (com.treef && com.treef[0]) {
      printf("trees go into %s.\n", com.treef);
      ftree = gfopen(com.treef, "w");
   }
   if (com.seqf && com.seqf[0]) {
      ...
      if (nsHet < nsFull) {
         ...
         p = strstr(line, "_full");
         sprintf(p, "_rand.txt");
         printf("Randomly resolved sequences are in %s\n", line);
         fseqrand = gfopen(line, "w");
      }
   }
   if (com.concatf && com.concatf[0] && com.seqf[0]) {
      printf("concatenated sequence alignment is in %s\n", com.concatf);
      fconcat = gfopen(com.concatf, "w");
      zconcat = (unsigned char**)malloc(nsHet * sizeof(unsigned char*));
      for (i = 0; i < nsHet; i++) {
         if ((zconcat[i] = (unsigned char*)malloc(com.ls*nloci*sizeof(unsigned char))) == NULL)
            error2("oom zconcat");
      }
   }
   ...
   for (i = 0, k = 0; i < stree.nspecies; i++)
      for (j = 0; j < stree.nseqsp[i]; j++, k) {
         sprintf(spnameFull[k++], "%s%d^%s", stree.nodes[i].name, j + 1, stree.nodes[i].name);
      }
   if (nsHet < nsFull) {
      for (i = 0, k = 0; i < stree.nspecies; i++) {
         if (data.diploid[i] == 0)
            for (j = 0; j < stree.nseqsp[i]; j++) 
               sprintf(spnameFull[k++], "%s%d^%s", stree.nodes[i].name, j + 1, stree.nodes[i].name);
         else
            for (j = 0; j < stree.nseqsp[i] / 2; j++) {
               sprintf(spnameFull[k++], "%s%da^%s", stree.nodes[i].name, j + 1, stree.nodes[i].name);
               sprintf(spnameFull[k++], "%s%db^%s", stree.nodes[i].name, j + 1, stree.nodes[i].name);
            }
      }
      assert(k == nsFull);
   }
   if (nsHet < nsFull) {
      for (i = 0, k = 0; i < stree.nspecies; i++) {
         for (j = 0; j < stree.nseqsp[i] / (data.diploid[i] ? 2 : 1); j++)
            sprintf(spnameHet[k++], "%s%d^%s", stree.nodes[i].name, j + 1, stree.nodes[i].name);
      }
   }
   for (i = 0; i < nsFull; i++)
      strcpy(com.spname[i], spnameFull[i]);
   ...
   for (locus = 0; locus < nloci; locus++) {
      ...
      for (i = 0; i < nsFull; i++) strcpy(com.spname[i], spnameFull[i]);
      ...
      if (fseq) {  /* evolve and print sequence alignment at locus */
         ...
         */
         if (nsHet < nsFull) {
            if (fseqfull)
               printSeqs(fseqfull, com.z, com.spname, nsFull, com.ls, 0, NULL, NULL, NULL, com.SeqFileFormat);
            if (fseqrand) {
               MakeDiploidRandom(com.z, zt, nsFull, com.ls);  /* this destroys com.z[] */
               printSeqs(fseqrand, zt, spnameFull, nsFull, com.ls, 0, NULL, NULL, NULL, com.SeqFileFormat);
            }
            MakeDiploid(com.z, zt, com.ns, nsHet, com.ls);
         }
         if (fconcat) {
            for (i = 0; i < nsHet; i++)
               memcpy(zconcat[i] + locus*com.ls, (nsHet < nsFull ? zt[i] : com.z[i]), com.ls * sizeof(unsigned char));
         }
   ...
   if (fconcat) {
      ...
      printSeqs(fconcat, zconcat, (nsHet<nsFull?spnameHet:spnameFull), nsHet, com.ls*nloci, 0, NULL, NULL, NULL, 0);
      fclose(fconcat);
      for (i = 0; i < nsHet; i++)
         free(zconcat[i]);
      free(zconcat);
   }
   for (i = 0; i < nsFull; i++)  free(spnameFull[i]);
   free(spnameFull);
   if (nsHet < nsHet) {
      for (i = 0; i < nsFull; i++)  free(spnameHet[i]);
      free(spnameHet);
   }

   if (ftree) fclose(ftree);
   if (fmodelpara) fclose(fmodelpara);
   if (fseq) {
      fclose(fseq);
      for (i = 0; i < 2 * nsFull - 1; i++)  free(com.z[i]);
      if (nsHet < nsFull) {
         fclose(fseqfull);
         fclose(fseqrand);
         for (i = 0; i < nsFull; i++)  free(zt[i]);
         free(zt);
      }
      if (data.fix_alpha == 0 || com.alpha) {
         free(data.siterates);
   ...
   for (i = 0; i < (stree.nspecies * 2 - 1)*(stree.nspecies * 2 - 1); i++)
      meanmigrations[i] /= nloci;
   if (stree.migration) {
      printf("\nCounts of migration events averaged over replicates: %8.4f\n", mtMRCA / nloci);
      matout(F0, meanmigrations, (stree.nspecies * 2 - 1), (stree.nspecies * 2 - 1));
   }
...
#else
      Tint[j] = 1 / Eff_IntegratedCorrelationTime2(y, n, (int)sqrt(n)/2, &mean[j], &var[j]); /* y destroyed */
#endif
